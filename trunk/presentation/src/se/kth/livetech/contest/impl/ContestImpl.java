...
import se.kth.livetech.contest.Language;
import se.kth.livetech.contest.Problem;
...
public class ContestImpl implements Contest {
    ...
    public Set<Integer> getTeams() { return teams.keySet(); }
    public Team getTeam(int i) { return teams.get(i); }
    public Set<Integer> getProblems() { return problems.keySet(); }
    public Problem getProblem(int i) { return problems.get(i); }
    public Set<String> getLanguages() { return languages.keySet(); }
    public Language getLanguage(String i) { return languages.get(i); }
    public Set<String> getJudgements() { return judgements.keySet(); }
    public Judgement getJudgement(String i) { return judgements.get(i); }
    public Set<Integer> getRuns() { return runs.keySet(); }
    public Run getRun(int i) { return runs.get(i); }
    public Set<Integer> getClars() { return clars.keySet(); }
    public Clar getClar(int i) { return clars.get(i); }
	...
	/*
	int index = Collections.binarySearch(ranking, team, teamComp);
	if (index < 0) index = -index - 1;
	while (index > 0 && ranking.get(index - 1).compareTo(team) == 0)
	    --index;
	return index + 1;
	*/
    }
